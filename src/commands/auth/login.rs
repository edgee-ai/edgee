...
pub async fn ensure_onboarded(provider: &str) -> Result<()> {
    ...
    // Bail out quietly if the prerequisites aren't in place — onboarding is a
    // convenience step and must never block the launch it runs ahead of.
    let has_auth = creds.user_token.as_deref().is_some_and(|t| !t.is_empty())
        && creds.org_id.as_deref().is_some_and(|o| !o.is_empty());
    if !has_auth {
        return Ok(());
    }
