...
/// as Kimi Code itself does) over the documented default `~/.kimi-code`.
/// **Not independently verified against a live install** — Kimi isn't
/// installed in the environment this was written in; taken from the cited
/// docs page.
...
pub(crate) fn tui_toml_path() -> Option<std::path::PathBuf> {
    kimi_code_home().map(|h| h.join("tui.toml"))
}
