...
impl BashCompressor for PrettierCompressor {
...
Code style issues found in the above file(s). Forgot to run Prettier?
    ...
    fn test_compressor_empty() {
        ...
        assert!(c.compress("npx prettier --check .", "").is_none());
    }

    #[test]
    fn test_compressor_skips_clean() {
        let c = PrettierCompressor;
        let output = "Checking formatting...\nAll matched files use Prettier code style.\n";
        let result = c.compress("npx prettier --check .", output);
        assert!(result.is_some());
        assert!(result.unwrap().contains("All files formatted correctly"));
    }
}
